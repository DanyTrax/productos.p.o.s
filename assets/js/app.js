...
// POS: barra de totales fija abajo en todo el viewport; sin slimScroll (scroll nativo + CSS).
function posCheckoutSlimScroll() {
  var $pl = $('#productList');
  if (!$pl.length) {
    return;
  }
  var $pl2 = $('#productList2');
  if ($pl.parent().hasClass('slimScrollDiv')) {
    $pl.slimScroll({ destroy: true });
  }
  if ($pl2.length && $pl2.parent().hasClass('slimScrollDiv')) {
    $pl2.slimScroll({ destroy: true });
  }
}
$(function () {
  posCheckoutSlimScroll();
});
$(window).on('resize', posCheckoutSlimScroll);
